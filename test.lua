-- [[ KRALLDEN SPY v9.5.0 - FULL FIX: ANTI-SPAM & VISIBILITY ]] --
...
local function formatTable(t, indent)
    ...
    for k, v in pairs(t) do
        ...
        if isArray then result = result .. spacing .. valStr .. ",\n"
        else
...
-- ОБНОВЛЕНИЕ ДЕТАЛЕЙ (ОБЩЕЕ ДЛЯ ВСЕХ)
local function updateDetailsText()
    ...
    if not data then 
        for path, d in pairs(ManualBannedPaths) do 
            if d.guid == currentSelectionGUID then data = d; break end 
        end 
    end
    ...
    if data then
        ...
        Details.Text = string.format("Type: %s%s\n\nPath: %s\n\nArgs: %s\n\nScript:\n%s:%s(%s)", 
            data.type, (ManualBannedPaths[data.path] and " (BANNED)" or ""), data.path, argDisplay, data.path, methodName, scriptArgs)
...
local function updateRedListUI()
    ...
    for path, data in pairs(ManualBannedPaths) do
        ...
        b.Text = " [AUTO-B] " .. (path:match("[^%.%[%]]+$") or path)
...
Title.Text = "KRALLDEN SPY v9.5.0"; Title.TextColor3 = Color3.new(1, 1, 1); Title.Font = Enum.Font.SourceSansBold; Title.TextSize = 16; Title.TextXAlignment = 0
...
local ControlBtn = createHeaderBtn("CONTROL: ON", -110, Color3.fromRGB(0, 170, 190))
local SelfBtn = createHeaderBtn("SELF: ON", -200, Color3.fromRGB(45, 90, 45), 80)
local DelBtn = createHeaderBtn("DEL BTN", -280, Color3.fromRGB(200, 100, 0), 70)
local AntiSpamBtn = createHeaderBtn("ANTI-SPAM: ON", -400, Color3.fromRGB(180, 150, 40))
AntiSpamBtn.Visible = false
local BlockBtn = createHeaderBtn("BLOCK EVENT", -510, Color3.fromRGB(150, 50, 50))
BlockBtn.Visible = false

ContentFrame = Instance.new("Frame", Main)
ContentFrame.Size = UDim2.new(1, 0, 1, -35); ContentFrame.Position = UDim2.new(0, 0, 0, 35); ContentFrame.BackgroundTransparency = 1
...
DetailsScroll.Position = UDim2.new(0, 205, 0, 8); DetailsScroll.Size = UDim2.new(0, 448, 0, 255); DetailsScroll.BackgroundColor3 = Color3.fromRGB(10, 10, 12); DetailsScroll.BorderSizePixel = 0; DetailsScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y

Details = Instance.new("TextBox", DetailsScroll)
Details.Size = UDim2.new(1, -10, 0, 0); Details.BackgroundTransparency = 1; Details.TextColor3 = Color3.new(1, 1, 1); Details.MultiLine = true; Details.TextWrapped = true; Details.Font = Enum.Font.Code; Details.TextSize = 12; Details.TextXAlignment = 0; Details.TextYAlignment = 0; Details.ClearTextOnFocus = false; Details.AutomaticSize = Enum.AutomaticSize.Y

local SortBtn = Instance.new("TextButton", ContentFrame)
SortBtn.Size = UDim2.new(0, 60, 0, 20); SortBtn.Position = UDim2.new(0, 590, 0, 12); SortBtn.BackgroundColor3 = Color3.fromRGB(40, 60, 150); SortBtn.Text = "SORT: OFF"; SortBtn.TextColor3 = Color3.new(1,1,1); SortBtn.Font = Enum.Font.SourceSansBold; SortBtn.TextSize = 10; SortBtn.BorderSizePixel = 0
...
local function addLog(rem, args, isSelf, typeLabel)
    ...
    local argList = {}
    for _, v in ipairs(args) do table.insert(argList, parseValue(v)) end
    local finalArgsStr = table.concat(argList, ", ")
    
    local alreadyExists = false
    for _, m in ipairs(MainMemory) do
        if m.path == eventPath and m.isSelf == isSelf then
            if (isSelf and selfMode) or (not isSelf and controlMode) or m.argsStr == finalArgsStr then 
    ...
    if not isSelf and not controlMode and antiSpam then
        ...
        if (now - (AntiSpamCooldowns[eventPath] or 0)) < 0.4 then
            ...
            if AntiSpamCounts[eventPath] >= 4 then
                local banData = { guid = generateGUID(), rawArgs = args, argsStr = finalArgsStr, type = typeLabel, path = eventPath }
                ManualBannedPaths[eventPath] = banData
                -- Удаляем из списка видимых
                local newMem = {}
                for _, m in ipairs(MainMemory) do if m.path ~= eventPath then table.insert(newMem, m) end end
                MainMemory = newMem
                task.spawn(updateRedListUI) -- Мгновенное обновление бан-листа
                return 
            end
...
mt.__namecall = newcclosure(function(self, ...)
    ...
    if not s and ManualBannedPaths[getSafePath(self)] then return nil end
    return old(self, ...)
...
SelfBtn.MouseButton1Click:Connect(function()
    ...
    SelfBtn.BackgroundColor3 = selfMode and Color3.fromRGB(45, 90, 45) or Color3.fromRGB(80, 80, 85)
end)
...
DelBtn.MouseButton1Click:Connect(function()
    if currentSelectionGUID then
        ...
        currentSelectionGUID = nil; Details.Text = ""; feedback(DelBtn, "DELETED")
    end
end)
...
print("KRALLDEN SPY v9.5.0 LOADED")
