...
-- Функция обновления холста (Фикс скролла)
local function updateCanvas()
    task.wait() -- Даем Roblox обновить TextBounds
    if Details and DetailsScroll then
        local textHeight = Details.TextBounds.Y
        Details.Size = UDim2.new(1, -10, 0, textHeight + 10)
...
-- ИСПРАВЛЕННАЯ ФУНКЦИЯ ОТРИСОВКИ (Использует ручной фикс скролла)
local function updateDetailsView()
    ...
    if content ~= "" then
        ...
        updateCanvas() -- Вызываем ручное обновление вместо task.defer
    end
end
...
-- Привязка обновления скролла к изменению текста
